...
**先行研究について、確かめたことと確かめていないことを分けて書く。**

```
半直積モノイド   既知（Zappa–Szép 積）  Mathlib は群版しか持たない
古典的な水平和   既知                  「horizontal sum」= **有界束** L, M の
                                     ⊥ どうし・⊤ どうしを**貼り合わせる**構成
                                     （lattice-based sums として族版もある）
この版           **未確認**
```

古典的な水平和とこの `Family.T` は**定義が違う**。古典版は既にある境界を潰し、
こちらは任意の半順序に境界を**新しく足す**。`P i` が既に有界なら結果は別物に
なる（向こうは `⊥_i` を消し、こちらは内部に残す）。

各 `P i` が束のとき、`Family.T ι P` は `{WithBot (WithTop (P i))}` に古典的な
水平和を適用したものに等しい。つまり**「境界を足す」と「古典的水平和」の合成**
に還元される。構成そのものは新しくない。

**`IsLUB` による特徴づけが文献に在るかは確かめていない。** 初等的なので
folklore だと思うが、それは見立てであって確認ではない。知っている人が居たら
教えてほしい。

はっきり言えるのは Mathlib の側だけである——`Sigma` に束は無く、`Sum.Lex` は
縦積み、`WithTop.lattice` は台が既に束であることを要求する。
